...
public final class HGHandler implements IRightHandler {
	...
	public boolean activateRight(final Set<HGHandle> parents,
		...
		HGHandle handle = findExistingHandle(children, true);
	 ...
	 */
	private HGHandle findExistingHandle(final Set<HGHandle> elements,
		...
		try {
			...
			// subsets.
			handles = hg.findAll(getHGDB(), hg.link(elements));
		...
		for (HGHandle tmpHandle : handles) {
			...
			if (headNotTail) {
				if (link.getHead().equals(elements)) {
					if (handle == null) {
						handle = tmpHandle;
					} else {
						throw new IllegalStateException(
				...
				if (link.getTail().equals(elements)) {
					if (handle == null) {
						handle = tmpHandle;
					} else {
						throw new IllegalStateException(
								"the set should only be contained once in the db");
					}
				}
			}
		}
